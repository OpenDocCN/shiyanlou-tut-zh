...
> **提示**：使用 `boost` 只是暂时性的解决方案，`variant` 已在 C++17 中被纳入标准库 `std::variant`，见扩展主题它的讨论。[`en.cppreference.com/w/cpp/utility/variant`](http://en.cppreference.com/w/cpp/utility/variant)
...
```cpp
wget http://labfile.oss.aliyuncs.com/courses/605/4.zip 
```