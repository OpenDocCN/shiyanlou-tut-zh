...
session 的运作通过一个 `session_id` 来进行。`session_id` 通常是存放在客户端的 cookie 中，比如在 express 中，默认是 `connect.sid` 这个字段，当请求到来时，服务端检查 cookie 中保存的 session*id 并通过这个 session*id 与服务器端的 session data 关联起来，进行数据的保存和修改。
...
<del> 上面一直提到 session 可以存在 cookie 中，现在来讲讲具体的思路。这里所涉及的专业名词叫做 对称加密。
...
2）是前者一般是长期保存，而后者是 session cookie </del>
...
初学者容易犯的一个错误是，忘记了 session*id 在 cookie 中的存储方式是 session cookie。即，当用户一关闭浏览器，浏览器 cookie 中的 session*id 字段就会消失。
